#!/usr/bin/env python3
"""
02_Classification - Document Classifier Runner (V6 - API, Filename Analysis & Batching)

This single script performs Stage 2 using a cloud API. It uses a highly
efficient "Hierarchy of Evidence" approach:
1. Tier 1: Classifies based on keywords in the FILENAME.
...
API_PROVIDER = "openai" # "openai", "anthropic", etc.
...
FILENAME_RULES = {
    DocumentType.INVOICE: ["invoice", "inv-"], DocumentType.RECEIPT: ["receipt"], DocumentType.PURCHASE_ORDER: ["purchase-order", "po-"],
    DocumentType.CREDIT_NOTE: ["credit-note", "credit_note"], DocumentType.BANK_STATEMENT: ["bank_statement", "bank-statement"],
    DocumentType.BALANCE_SHEET: ["balance sheet", "balance_sheet"], DocumentType.PROFIT_AND_LOSS: ["profit and loss", "p&l"],
    DocumentType.CONTRACT: ["contract", "agreement"], DocumentType.QUOTATION: ["quote", "quotation"],
}

CONTENT_RULES = {
    DocumentType.INVOICE: ["invoice", "bill to", "remittance"], DocumentType.RECEIPT: ["receipt", "cash sale", "payment confirmation"],
    DocumentType.PURCHASE_ORDER: ["purchase order", "po number", "ship to"], DocumentType.CREDIT_NOTE: ["credit note", "credit memo"],
    DocumentType.BANK_STATEMENT: ["bank statement", "account summary"], DocumentType.BALANCE_SHEET: ["balance sheet", "assets", "liabilities"],
    DocumentType.PROFIT_AND_LOSS: ["profit and loss", "income statement"], DocumentType.CONTRACT: ["agreement", "contract", "parties"],
    DocumentType.RESUME: ["resume", "curriculum vitae"], DocumentType.BILL_OF_LADING: ["bill of lading", "bol"],
    DocumentType.PACKING_SLIP: ["packing slip", "packing list"], DocumentType.UTILITY_BILL: ["utility bill", "gas bill"],
}

class FastDocumentClassifier:
    """A hybrid classifier using a tiered 'Hierarchy of Evidence' approach with a cloud API."""
    def __init__(self, use_llm_fallback=True):
        ...
        if not self.api_config or not self.api_config.get("api_key"):
            raise ValueError(f"API configuration for '{API_PROVIDER}' is missing or API key not found in .env file.")
    ...
    def classify_with_rules(self, text: str, filename: str, confidence_threshold: float) -> Tuple[Any, float, Dict] | None:
        ...
        if max_score >= 3: return best_match, 0.95, {"classification_method": "rules_content_high"}
    ...
    def classify_with_llm(self, text: str) -> (str, float, dict):
        ...
        prompt = f"""You are an expert document analyst. Analyze the following text and return a concise document type. Examples: "Invoice", "Bank Statement", "Employment Contract". Respond with ONLY a valid JSON object in this exact format: {{"predicted_type": "The concise document type", "confidence": 0.9, "reasoning": "A brief explanation for the classification.", "key_indicators": ["list", "of", "key", "words"]}} TEXT: "{truncated_text}" """
        
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {self.api_config['api_key']}"}
        payload = {"model": self.api_config['model'], "messages": [{"role": "user", "content": prompt}], "temperature": 0.1, "response_format": {"type": "json_object"}}

        try:
            logging.info(f"Engaging API LLM ({self.api_config['model']})...")
            response = requests.post(self.api_config['url'], headers=headers, json=payload, timeout=60)
            ...
            return data.get("predicted_type", "API_Parse_Error"), float(data.get("confidence", 0.5)), metadata
        except Exception as e:
            logging.error(f"API LLM classification failed: {e}")
            return "API_Connection_Error", 0.0, {"classification_method": "llm_generative", "error": str(e)}
...
def main():
    parser = argparse.ArgumentParser(description='Stage 2: Document Classifier Runner (API, Batching)')
    ...
    classifier = FastDocumentClassifier(use_llm_fallback=USE_LLM_FALLBACK)
    logger.info(f"Classifier initialized for API: {API_PROVIDER}. LLM Fallback is {'ENABLED' if USE_LLM_FALLBACK else 'DISABLED'}.")
